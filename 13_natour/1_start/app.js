...
app.all('*', (req, res, next) => {
  ...
  next(new AppError(`No such route ${req.originalUrl} is available`, 404)); // Since we are passing argument inside "next()" function , so it will directly call the "global error middleware function" and it will escape all the middleware function in between the "global middleware" function and itself, if there is any middleware remaining in between them which is yet to be executed.
});
...
//!==> Here we are refactoring our code for error. By keeping our global error handler middleware in a separete folder, instead of defining the whole function, and we have created a separate usuable error handler, which will handle all the errors and set the error message and status mode i.e. "AppError"
